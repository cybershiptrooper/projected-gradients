...
def convert_fsdp_name(name):
    subtr_to_remove = "._checkpoint_wrapped_module."
    if subtr_to_remove in name:
        name = name.replace(subtr_to_remove, ".")
    return name
    

def project_gradients(model, projections, train_config):
    def project_fn():
        for name, param in model.named_parameters():
            name = convert_fsdp_name(name)
            if name in projections.keys():
                # print(f"Projecting gradients for {name}")
...
def train(model, train_dataloader,eval_dataloader, tokenizer, optimizer, lr_scheduler, gradient_accumulation_steps, train_config, fsdp_config=None, local_rank=None, rank=None, projections=None):
    """
    ...
    Args:
        ...
        project_gradients: Whether to project the gradients before applying them to the model
    
    Returns: results dictionary containing average training and validation perplexity and loss
    """
    if train_config.project_gradients:
        assert projections is not None, "Projections must be provided if project_gradients is True"

        def sanity_check_fn():
            n_exists_in_projections = 0
            for n, _ in model.named_parameters():
                n = convert_fsdp_name(n)
                if n in projections.keys():
                    n_exists_in_projections += 1
                    print(f"Parameter {n} has a projection")
                else:
                    print(f"Parameter {n} does not have a projection")
            print(f"Number of parameters that have a projection: {n_exists_in_projections}")
            print(f"Number of parameters in the model: {len(list(model.parameters()))}")
            assert n_exists_in_projections == len(list(projections.keys())), "All parameters must have a projection"
            ...
            # for step, batch in enumerate(tqdm(train_dataloader,colour="blue", desc=f"Training Epoch{epoch}")):
                ...
                if train_config.use_fp16:
                    ...
                    if (step + 1) % gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                        ...
                        if train_config.project_gradients:
                            # store gradients before projection
                            grads = {}
                            for name, param in model.named_parameters():
                                grads[name] = param.grad.detach().clone().cpu()
                            
                            print("Projecting the gradients")
                            project_gradients(model, projections, train_config)
                            print("Gradients projected")

                            new_grads = {}
                            for name, param in model.named_parameters():
                                new_grads[name] = param.grad.detach().clone().cpu()
                        
                            # check if the gradients have been projected correctly
                            for name in grads.keys():
                                grads_are_same = torch.allclose(grads[name], new_grads[name], atol=1e-6)
                                if grads_are_same:
                                    print(f"Gradients for {name} are the same")
