...
def make_projections(train_config):
    print("-" * 100)
    print("Making projections")
    print("-" * 100)
    device_map = "auto"
    model = AutoModelForCausalLM.from_pretrained(train_config.model_name, device_map=device_map, torch_dtype=torch.float32)
    model.requires_grad_(False)
    base_model = AutoModelForCausalLM.from_pretrained(train_config.base_model_name, device_map=device_map, torch_dtype=torch.float32)
    ...
    projections = SVDProjectionStore.make_projection_store(
        ndim=train_config.ndim,
        names_of_params=params,
        sft_model=model,
        it_model=base_model,
        projection_type="both",
        keep_in_files=False,
        dump_root_dir=None # f"results/{train_config.model_name}/projections",
    )
    print("-" * 100)
    print("Done making projections")
    print("-" * 100)
    return projections
...
def main(**kwargs):
    ...
    if train_config.project_gradients:
        projections = make_projections(train_config)
    else:
        projections = None
    ...
    if train_config.quantization:
        raise NotImplementedError("Quantization is not supported in this version of Llama")
